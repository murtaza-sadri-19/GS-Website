    ...
    <div className="space-y-5">
      ...
      {!published && !loading && (
        <PortalCard className="!p-3 !bg-accent/10 !border-accent/30">
          <p className="text-xs text-accent font-bold">The exam schedule has not been published yet. Check back later or contact the Exam Department.</p>
        </PortalCard>
      )}
      ...
      <PortalCard className="!p-3">
        <div className="flex items-center gap-3">
          ...
          <select value={String(semFilter)} onChange={e => setSemFilter(e.target.value === 'all' ? 'all' : Number(e.target.value))}
            className="border border-slate-200 rounded px-3 py-1.5 text-sm bg-white focus:outline-none focus:border-primary">
            ...
            {visiblePdfs.map(p => (
              <div key={p.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">{p.branch_id} · Sem {p.semester} · {p.type} · {p.dateAdded}</p>
                </div>
