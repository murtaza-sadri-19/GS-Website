...
const TNPCell: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ]).then(([ys, t, proc, train, part, info]) => {
    ...
    <div className="space-y-10">
      ...
      {loading ? (
        ...
        <div className="bg-primary rounded-2xl p-6 text-white">
          <p className="text-xs uppercase tracking-widest font-bold text-accent/80 mb-3">Placement Statistics {latest.year}</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            {[
      ...
      {/* T&P Team */}
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Our Team</span>
            ...
            <div key={i} className={`bg-white rounded-xl border p-5 shadow-sm hover:shadow-md transition-all ${i === 0 ? 'border-accent/40 ring-1 ring-accent/20' : 'border-slate-200'}`}>
              {i === 0 && (
                <div className="flex items-center gap-1 mb-3">
                  ...
                  <span className="text-xs text-accent font-bold uppercase tracking-wider">Lead</span>
                </div>
              )}
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-accent font-bold uppercase tracking-wider mt-0.5">{member.title}</p>
                  <p className="text-xs text-slate-400 font-medium mt-0.5">{member.exp} experience</p>
                </div>
              </div>
              <p className="text-xs text-slate-500 font-medium mb-3 font-sans">{member.dept}</p>
      ...
      {/* Placement Process */}
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Process</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {process.map((step, i) => (
            ...
            >
              ...
              <p className="text-xs text-slate-500 leading-relaxed font-medium font-sans">{step.desc}</p>
              {activeStep === i && (
      ...
      {/* Training Programs */}
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Training</span>
        <h3 className="text-xl font-display font-bold text-slate-900 mb-4">Year-Round Training Programs</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {training.map((prog, i) => (
            <div key={`training-${i}`} className="flex items-start gap-3 bg-white border border-slate-200 rounded-xl p-3.5 text-sm hover:border-accent/30 hover:bg-accent/5 transition-all">
      ...
      {/* Partners */}
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Partners</span>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-2.5">
          {partners.map((company, i) => (
            <div
              key={`partner-${i}`}
              className="bg-white border border-slate-200 rounded-xl px-2 py-3 text-center hover:border-accent/40 hover:bg-accent/5 hover:shadow-sm transition-all cursor-pointer"
            >
              <p className="text-xs font-bold text-primary font-sans leading-tight">{company}</p>
            </div>
          ))}
        </div>
      </div>
