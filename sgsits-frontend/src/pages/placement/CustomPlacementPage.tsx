    ...
    return (
      <div className="space-y-8 bg-white py-12 text-center max-w-xl mx-auto">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-display font-bold text-slate-800">Placement Page Not Found</h2>
          <p className="text-sm text-slate-500 leading-relaxed font-sans">
            This route has been registered in the Placements navigation tree, but its page content has not been drafted inside the Central CMS workspace yet.
          </p>
        </div>
        <div className="pt-4 flex items-center justify-center gap-3">
          ...
          <Link
            to="/dashboard/placement/cms"
            className="px-4 py-2 border border-slate-200 text-slate-700 text-xs font-bold uppercase tracking-wider rounded-md hover:bg-slate-50 font-sans font-semibold"
          >
            Open Placements CMS
          </Link>
        </div>
  ...
  return (
    <div className="space-y-12 bg-white">
      ...
      <div className="border-b border-slate-200 pb-5">
        <span className="text-[10px] uppercase font-bold tracking-widest text-accent block mb-1">Placement Cell</span>
      ...
      {data.narrativeParagraphs && data.narrativeParagraphs.length > 0 && (
        <div className="border-l-2 border-accent pl-6">
          <div className="text-slate-655 space-y-4 text-sm leading-relaxed font-sans text-justify">
            {data.narrativeParagraphs.map((para: string, index: number) => (
              <p
                key={index}
                className={index === 0 ? "text-base text-slate-800 leading-relaxed font-medium" : "text-slate-655"}
      ...
      {data.highlights && data.highlights.length > 0 && (
        <div className="space-y-6 pt-2">
          <div>
            ...
            <h3 className="text-xl font-display font-bold text-slate-900">Key Information</h3>
      ...
      {data.affiliations && data.affiliations.length > 0 && (
        <div className="bg-slate-50 rounded border border-slate-200/80 p-6 border-l-2 border-l-[#0b2545]">
          <div className="space-y-6">
            <div>
              <span className="text-[10px] uppercase font-bold tracking-widest text-accent block mb-1">Placement Desk Guidelines</span>
              <h3 className="text-xl font-display font-bold text-slate-900">Key Protocols & Criteria</h3>
              <p className="text-xs text-slate-500 mt-1 font-sans font-medium">Please review before participating in corporate drives</p>
            </div>
