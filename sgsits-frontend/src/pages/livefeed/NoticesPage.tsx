...
import { Search, FileText, ChevronLeft, ChevronRight, Tag, Eye, RefreshCw } from 'lucide-react'
...
const fetchNotices = () => getNotices()

const NoticesPage: React.FC = () => {
  const { data: notices, loading, isFetching, refresh } = usePageData<any[]>('notices', fetchNotices)

  // UI state — initialised from persisted Zustand store so it survives navigation
  const noticesUI    = usePageCacheStore(s => s.noticesUI)
  const setNoticesUI = usePageCacheStore(s => s.setNoticesUI)

  const [activeTab, setActiveTab] = useState<CategoryFilter>(noticesUI.category as CategoryFilter)
  const [search, setSearch]       = useState(noticesUI.search)
  const [page, setPage]           = useState(noticesUI.page)
  ...
  const handleTabChange = (tab: CategoryFilter) => {
    setActiveTab(tab)
    setPage(1)
    setNoticesUI({ category: tab, page: 1 })
  }
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearch(val)
    setPage(1)
    setNoticesUI({ search: val, page: 1 })
  }
        ...
        <div className="max-w-6xl mx-auto px-6 py-12">
          ...
          <div className="flex items-end justify-between gap-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-bold text-white/60 hover:text-white transition-colors disabled:opacity-40 shrink-0 mb-2"
            >
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
          ...
          <span className="text-sm text-slate-500 font-medium shrink-0">
            {isFetching ? '...' : `${filtered.length} notice${filtered.length !== 1 ? 's' : ''} found`}
          </span>
        </div>
            ...
            >
              ...
              {tab !== 'all' && !loading && (
                <span className="ml-1.5 opacity-70">
                  ({allNotices.filter(n => n.category === tab).length})
                </span>
              )}
            </button>
          ))}
