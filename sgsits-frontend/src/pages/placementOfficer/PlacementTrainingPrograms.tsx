...
const PlacementTrainingPrograms: React.FC = () => {
  const [programs, setPrograms]   = useState<TrainingRecord[]>([])
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await apiClient.get('/v1/placement/training-programs', { params: { pageSize: 100 } })
      setPrograms(res.data?.data?.records ?? [])
    } catch { setToast('Failed to load training programs.') }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post('/v1/placement/records', {
        ...
        company_name: trainer.trim() || null,
        academic_year: academicYear || null,
        description: description.trim() || null,
      })
      setToast('Training program added.'); setShowModal(false); load()
    } catch { setToast('Failed to add training program.') }
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle="Manage aptitude training, mock interviews, and pre-placement workshops"
        action={
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setTitle(''); setTrainer(''); setAcademicYear('2025-26'); setDescription(''); setShowModal(true) }}
              ...
              <Plus size={15}/><span>Add Program</span>
            </button>
          </div>
        }
      />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Program Title</th>
                <th className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Conducted By</th>
                <th className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Session</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.length === 0 ? (
                  <tr><td colSpan={4} className="text-center py-8 text-slate-400 text-xs">
                    {programs.length === 0 ? 'No training programs recorded yet.' : 'No results match your search.'}
                  </td></tr>
                ) : filtered.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-slate-600 font-semibold">{p.company_name ?? '—'}</td>
                    <td className="px-4 py-3 text-xs text-slate-500">{p.academic_year ?? '—'}</td>
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => handleDelete(p.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="Delete">
                        <Trash2 size={13}/>
                      </button>
                    </td>
                  </tr>
      ...
      <PortalModal isOpen={showModal} title="Add Training Program" onClose={() => setShowModal(false)}>
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Program Title *</label>
            <input type="text" required value={title} onChange={e => setTitle(e.target.value)}
              placeholder="e.g. Aptitude & Reasoning Boot Camp"
              className="w-full border border-slate-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-primary"/>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Conducted By</label>
              <input type="text" value={trainer} onChange={e => setTrainer(e.target.value)}
                placeholder="e.g. TCS iON"
                className="w-full border border-slate-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-primary"/>
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Academic Year</label>
              <select value={academicYear} onChange={e => setAcademicYear(e.target.value)}
                className="w-full border border-slate-200 rounded px-3 py-2 text-sm bg-white focus:outline-none">
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
              placeholder="Duration, topics covered, target batch..."
