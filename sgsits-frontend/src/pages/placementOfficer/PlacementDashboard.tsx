...
const PlacementDashboard: React.FC = () => {
  ...
  const kpiCards = [
    { label: 'Students Placed',   value: stats?.totalPlaced ?? '—',        icon: Users,       color: 'bg-primary/10 text-primary border-primary/25', link: '/dashboard/placement/records', desc: 'Academic year so far' },
    { label: 'Companies Visited', value: stats?.companiesVisited ?? '—',   icon: Building2,   color: 'bg-accent/15 text-accent border-accent/40', link: '/dashboard/placement/company-visits', desc: 'This academic year' },
    { label: 'Highest Package',   value: stats?.highestPackage ?? '—',     icon: IndianRupee, color: 'bg-accent/20 text-accent border-accent/40', link: '/dashboard/placement/records', desc: 'Best offer this year' },
    { label: 'Average Package',   value: stats?.averagePackage ?? '—',     icon: TrendingUp,  color: 'bg-primary/10 text-primary border-primary/20', link: '/dashboard/placement/records', desc: 'Across all branches' },
    { label: 'Company Visits',    value: visits.length || '—',             icon: Calendar,    color: 'bg-primary/15 text-primary border-primary/30', link: '/dashboard/placement/company-visits', desc: 'Logged in system' },
    { label: 'Internships',       value: '—',                              icon: Briefcase,   color: 'bg-primary/5 text-primary border-primary/15',  link: '/dashboard/placement/internships', desc: 'Recorded' },
    { label: 'Training Programs', value: '—',                              icon: GraduationCap, color: 'bg-accent/10 text-accent border-accent/30', link: '/dashboard/placement/training-programs', desc: 'Organized' },
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div>
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Placement Pulse</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {kpiCards.map(card => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <PortalCard>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/dashboard/placement/company-visits" className="text-xs text-primary hover:underline font-medium">Manage &rarr;</Link>
          </div>
          {visits.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {visits.slice(0, 3).map(v => (
                <div key={v.id} className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                  ...
                  {v.company_name && <p className="text-xs text-slate-500 mt-0.5">{v.company_name}</p>}
                  <p className="text-xs text-slate-400 mt-1">{v.created_at?.slice(0, 10)}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <PortalCard>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/dashboard/placement/records" className="text-xs text-primary hover:underline font-medium">View all &rarr;</Link>
          </div>
          {recent.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200">
                    <th className="text-left px-2 py-2 text-xs font-bold text-slate-500 uppercase">Student</th>
                    <th className="text-left px-2 py-2 text-xs font-bold text-slate-500 uppercase">Branch</th>
                    <th className="text-left px-2 py-2 text-xs font-bold text-slate-500 uppercase">Company</th>
                    <th className="text-right px-2 py-2 text-xs font-bold text-slate-500 uppercase">CTC</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {recent.map(p => (
                    <tr key={p.id} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-2 py-2 text-xs text-right font-bold text-accent">
