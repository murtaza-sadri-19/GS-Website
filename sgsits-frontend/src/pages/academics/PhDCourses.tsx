...
const PhDCourses: React.FC<{ previewData?: any }> = ({ previewData }) => {
  ...
  useEffect(() => {
    if (!previewData) {
      academicsService.getPhDCourses().then(setFetchedData).catch(() => {})
    }
  }, [previewData])
  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Research</span>
      ...
      {((phdData.eligibilityPoints ?? []).length > 0 || (phdData.fellowshipRows ?? []).length > 0) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {(phdData.eligibilityPoints ?? []).length > 0 && (
            <div className="bg-slate-50 border border-slate-200 rounded p-5">
              ...
              <div className="space-y-2 text-sm text-slate-600 font-sans">
                {(phdData.eligibilityPoints as string[]).map((item, i) => (
          ...
          {(phdData.fellowshipRows ?? []).length > 0 && (
            <div className="bg-white border border-slate-200 rounded p-5">
              ...
              <div className="space-y-2 text-sm text-slate-600 font-sans">
                {(phdData.fellowshipRows as { label: string; value: string; accent?: boolean; note?: string }[]).map((row, i, arr) => (
                  row.note
                    ? <p key={i} className="text-xs text-slate-400 mt-2 font-sans">{row.note}</p>
                    : (
                      <div key={i} className={`flex justify-between py-1 ${i < arr.length - 1 ? 'border-b border-slate-100' : ''}`}>
                        <span>{row.label}</span>
                        <span className={`font-semibold ${row.accent ? 'text-accent' : 'text-slate-700'}`}>{row.value}</span>
                      </div>
                    )
                ))}
              </div>
      ...
      {/* Admission Process */}
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Admission</span>
