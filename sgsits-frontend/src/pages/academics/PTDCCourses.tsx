  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border-b border-slate-200 pb-5">
        ...
        <p className="text-sm text-slate-500 mt-1 font-medium">Part-Time Degree Courses for working professionals</p>
      </div>

      {(ptdcData as any).introParagraphs?.length > 0 && (
        <div className="space-y-4">
          {((ptdcData as any).introParagraphs as string[]).map((para, i) => (
          ...
          {ptdcData.map((course, idx) => (
            <div key={idx} className="bg-white border border-slate-200 rounded-md p-5 shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col justify-between">
              <div>
                ...
                <p className="text-slate-500 text-xs font-bold mt-1 uppercase tracking-wider font-sans">{course.dept}</p>
