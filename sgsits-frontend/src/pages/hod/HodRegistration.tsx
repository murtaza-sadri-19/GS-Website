...
const HodRegistration: React.FC = () => {
  ...
  const decide = async (decision: 'approved' | 'rejected') => {
    if (!reviewing || deciding) return
    setDeciding(true)
    try {
      const action = decision === 'approved' ? 'approve' : 'reject'
      await apiClient.put(`/v1/registration-requests/${reviewing.id}/${action}`, {
      ...
      <PortalCard className="!p-3">
        <div className="flex flex-col sm:flex-row gap-2.5">
          <div className="relative flex-1 min-w-0">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name / email / ID..." className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded focus:outline-none focus:border-primary" />
          </div>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as 'all' | RegistrationRequest['status'])} className="border border-slate-200 rounded px-3 py-2 text-sm bg-white focus:outline-none focus:border-primary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {visible.map(r => (
            <PortalCard key={r.id}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{r.designation}</p>
                  </div>
                </div>
              ...
              <p className="text-xs text-slate-400 mt-3">Applied on {r.appliedOn}</p>

              {r.status === 'pending' && (
                <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                  <button onClick={() => { setReviewing(r); setRemark('') }} className="flex-1 py-1.5 bg-primary text-white text-xs font-bold rounded hover:bg-primary/90 inline-flex items-center justify-center gap-1.5">
        ...
        {reviewing && (
          <div className="space-y-3">
            ...
            <label className="block">
              <span className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Remarks (optional)</span>
              <textarea rows={2} value={remark} onChange={(e) => setRemark(e.target.value)} className="w-full border border-slate-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-primary bg-white" placeholder="Optional note attached to your decision" />
            </label>
            <div className="flex gap-2 pt-2 border-t border-slate-100">
              <button onClick={() => decide('rejected')} disabled={deciding} className="flex-1 py-2 border border-primary/20 text-primary text-sm font-bold rounded hover:bg-primary/5 inline-flex items-center justify-center gap-1.5 disabled:opacity-60">
                <X size={13} /> {deciding ? 'Processing…' : 'Reject'}
              </button>
              <button onClick={() => decide('approved')} disabled={deciding} className="flex-1 py-2 bg-accent text-white text-sm font-bold rounded hover:bg-accent/90 inline-flex items-center justify-center gap-1.5 disabled:opacity-60">
                <Check size={13} /> {deciding ? 'Processing…' : 'Approve'}
              </button>
            </div>
          </div>
        )}
...
const Line: React.FC<{ icon: React.ComponentType<{ size?: number; className?: string }>; children: React.ReactNode }> = ({ icon: Icon, children }) => (
  <p className="flex items-center gap-2 text-xs text-slate-600"><Icon size={11} className="text-accent shrink-0" />{children}</p>
)
...
const StatusPill: React.FC<{ status: RegistrationRequest['status'] }> = ({ status }) => {
  const cls = status === 'approved' ? 'bg-accent/10 text-accent border-accent/30' :
              status === 'pending'  ? 'bg-accent/15 text-accent border-accent/40' :
