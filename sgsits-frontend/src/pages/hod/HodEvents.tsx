...
import { getHodEvents, createHodEvent, updateHodEvent, deleteHodEvent, type HodEvent } from '../../services/hodService'
...
const HodEvents: React.FC = () => {
  ...
  const load = () => getHodEvents(deptId).then(setEvents).catch(() => {}).finally(() => setLoading(false))
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) { await updateHodEvent(editing.id, form); showToast(`Event "${form.title}" updated.`) }
      else { await createHodEvent(form); showToast(`Event "${form.title}" created.`) }
      await load()
    } catch { showToast('Failed to save.') }
    setShowForm(false); setEditing(null); setForm(EMPTY)
  }

  const publish = async (id: string) => { try { await updateHodEvent(id, { status: 'published' }); showToast('Event published.'); await load() } catch { showToast('Failed.') } }
  const archive = async (id: string) => { try { await updateHodEvent(id, { status: 'archived' }); showToast('Event archived.'); await load() } catch { showToast('Failed.') } }
  const handleDelete = async () => { if (!deleteTarget) return; try { await deleteHodEvent(deleteTarget.id); showToast(`Deleted "${deleteTarget.title}".`); await load() } catch { showToast('Failed.') }; setDeleteTarget(null) }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Stat label="Upcoming" value={stats.upcoming} accent="text-accent" />
      ...
      <PortalCard className="!p-3">
        <div className="flex flex-col sm:flex-row gap-2.5">
          <div className="relative flex-1 min-w-0">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by event title..." className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded focus:outline-none focus:border-primary" />
          </div>
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value as 'all' | HodEvent['event_type'])} className="border border-slate-200 rounded px-3 py-2 text-sm bg-white focus:outline-none focus:border-primary">
            <option value="all">All Types</option>
            {TYPES.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as 'all' | HodEvent['status'])} className="border border-slate-200 rounded px-3 py-2 text-sm bg-white focus:outline-none focus:border-primary">
            <option value="all">All Statuses</option>
            {STATUSES.map(s => <option key={s} value={s}>{s[0].toUpperCase() + s.slice(1)}</option>)}
          </select>
          <div className="inline-flex border border-slate-200 rounded overflow-hidden">
            <button onClick={() => setView('grid')} className={`px-3 py-2 text-xs font-bold ${view === 'grid' ? 'bg-primary text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>Grid</button>
            <button onClick={() => setView('table')} className={`px-3 py-2 text-xs font-bold ${view === 'table' ? 'bg-primary text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>Table</button>
          </div>
        </div>
      </PortalCard>
          ...
          {visible.length === 0 ? (
            ...
            <PortalCard key={ev.id} className="!p-0 overflow-hidden">
              <div className="h-24 bg-gradient-to-br from-[#0b2545] to-[#0b2545]/70 relative flex items-end p-3">
                <span className="text-xs font-bold px-2 py-0.5 rounded bg-white/15 text-white border border-white/20 uppercase tracking-wide">{ev.event_type}</span>
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="flex items-center gap-1 pt-2 border-t border-slate-100">
                  ...
                  <span className="ml-auto text-xs text-slate-400 uppercase">{ev.audience}</span>
                </div>
              </div>
            </PortalCard>
          ))}
                ...
                {visible.length === 0 ? <tr><td colSpan={8} className="px-4 py-8 text-center text-sm text-slate-400">No events match.</td></tr> :
                  visible.map(ev => (
                    <tr key={ev.id} className="hover:bg-slate-50/60">
                      <td className="px-4 py-2.5"><p className="text-sm font-semibold text-slate-800">{ev.title}</p><p className="text-xs text-slate-500 line-clamp-1">{ev.description}</p></td>
                      <td className="px-4 py-2.5"><span className="text-xs font-bold px-2 py-0.5 rounded bg-primary/5 text-primary border border-primary/15 uppercase tracking-wide">{ev.event_type}</span></td>
      ...
      <PortalModal isOpen={showForm} title={editing ? `Edit Event — ${editing.id}` : 'Create Department Event'} onClose={() => { setShowForm(false); setEditing(null) }} width="max-w-xl">
        <form onSubmit={save} className="space-y-3">
          ...
          <div className="flex gap-2.5 pt-2 border-t border-slate-100">
            ...
            <button type="submit" className="flex-1 py-2 bg-primary text-white text-sm font-bold rounded hover:bg-primary/90">{editing ? 'Update' : 'Create'}</button>
          </div>
        </form>
      </PortalModal>
...
const Meta: React.FC<{ icon: React.ComponentType<{ size?: number; className?: string }>; children: React.ReactNode }> = ({ icon: Icon, children }) => (
  <p className="flex items-center gap-1.5 text-xs text-slate-600"><Icon size={11} className="text-accent" />{children}</p>
)
