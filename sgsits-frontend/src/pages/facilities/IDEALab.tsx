...
const IDEALab: React.FC<{ previewData?: any }> = ({ previewData }) => {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Focus Areas */}
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Focus Areas</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(data.focusAreas || []).map((area: any) => (
            <div key={area.title} className="bg-white border border-slate-200 rounded p-4 hover:border-slate-400 transition-colors">
              ...
              <p className="text-xs text-slate-500 font-medium font-sans leading-relaxed">{area.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-primary text-white rounded p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          {data.accessNote && <p className="text-sm text-slate-300 mt-1">{data.accessNote}</p>}
