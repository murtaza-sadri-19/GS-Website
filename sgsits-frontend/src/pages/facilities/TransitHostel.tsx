...
const TransitHostel: React.FC<{ previewData?: any }> = ({ previewData }) => {
  const [fetchedData, setFetchedData] = useState<any>(null)
  useEffect(() => { if (!previewData) getTransitHostel().then(setFetchedData) }, [previewData])
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Amenities */}
      <div>
        <span className="text-xs uppercase font-bold tracking-widest text-accent block mb-1">Room Facilities</span>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded p-5">
        ...
        {(data.bookingSteps || []).length > 0 ? (
          <ol className="space-y-2 text-sm font-sans text-slate-600">
            {(data.bookingSteps as string[]).map((step: string, i: number) => (
        ...
        {data.priorityNote && <p className="text-xs text-slate-400 mt-3 font-medium italic">{data.priorityNote}</p>}
