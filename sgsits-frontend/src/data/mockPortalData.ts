/**
 * Mock data for Exam Dept / HOD / Faculty portals.
...
// ── Registration Requests (pending HOD approval) ──────────────────────────
...
export const MARKS_REQUESTS: MarksRequest[] = [
...
// ── Correction Requests (faculty → admin) ──────────────────────────────
...
export const ELECTIVE_SUBJECTS: ElectiveSubject[] = [
  { id: 'CS502', name: 'Machine Learning',           type: 'Elective', semester: 7, branch_id: 'CSE', uploadStatus: 'uploaded', uploadedOn: '2026-05-10' },
  { id: 'CS503', name: 'Cloud Computing',             type: 'Elective', semester: 7, branch_id: 'CSE', uploadStatus: 'pending' },
  { id: 'CS504', name: 'Natural Language Processing', type: 'Elective', semester: 7, branch_id: 'CSE', uploadStatus: 'pending' },
  { id: 'CS505', name: 'Cyber Security',              type: 'Elective', semester: 7, branch_id: 'CSE', uploadStatus: 'uploaded', uploadedOn: '2026-05-08' },
]
...
export function generateMockMarks(subject: Subject, section: string, component: string): StudentMark[] {
  const students = STUDENTS.filter(s => s.branch_id === subject.branch_id && s.semester === subject.semester && s.section === section)
  const maxMap: Record<string, number> = { 'MST 1': 20, 'MST 2': 20, 'Assignment 1': 10, 'Assignment 2': 10, 'Theory Exam': 80, 'Viva 1': 25, 'Viva 2': 25, 'External Viva': 30, 'External Submission': 20 }
  const max = maxMap[component] ?? 20
