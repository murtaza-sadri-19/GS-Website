...
async function listLogs({
  page = 1, pageSize = 50,
  user_id, action, module_name, severity,
  ...
  if (search) {
    conditions.push('(al.description LIKE ? OR al.entity_name LIKE ? OR u.name LIKE ? OR u.email LIKE ?)');
    const q = `%${search}%`;
    params.push(q, q, q, q);
  }
  ...
  const [[rows], [countRows]] = await Promise.all([
    ...
    pool.execute(
      `SELECT COUNT(*) AS total FROM audit_logs al INNER JOIN users u ON al.user_id = u.id ${where}`,
      params
    ),
  ]);
...
// Returns distinct action/module/severity values for filter dropdowns
async function getFilterOptions() {
  const [[actions], [modules], [severities]] = await Promise.all([
...
async function getStats() {
  ...
  const [[total], [recent], [critical], topUsers, topModules] = await Promise.all([
    ...
    pool.execute(`
      ...
      WHERE al.created_at >= ?
      GROUP BY al.user_id ORDER BY cnt DESC LIMIT 5`, [last24h]),
    pool.execute(`
      SELECT module_name, COUNT(*) AS cnt
      FROM audit_logs WHERE created_at >= ?
      GROUP BY module_name ORDER BY cnt DESC LIMIT 5`, [last24h]),
  ]);
...
async function getRecentActivity(limit = 15) {
  const [rows] = await pool.execute(
    `SELECT ${AUDIT_COLS} ${FROM_CLAUSE} ORDER BY al.created_at DESC LIMIT ?`,
    [limit]
  );
